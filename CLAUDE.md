...
- **Graceful degradation on missing env**: if `CONSUMER_ENABLED !== 'true'`, or `STABLEKRAFT_BASE_URL` is missing, or `HIVE_ACCOUNT_NAME` is missing → consumer logs `[consumer] idling: <reason>` and sits in a `setInterval` loop. It does **not** exit. This is important because `entrypoint.sh` supervises with `wait -n` — any child exit terminates the container. Before this change, a missing consumer env var took down the pusher and Caddy too, creating a restart loop. The idle path keeps pusher healthy regardless of consumer config. Required for any future child process added under the same supervisor.
